...
void imprime_tabuleiro(int tab[4][4], int* recorde, int* pont, ALLEGRO_DISPLAY* janela) {
    ...
    al_draw_filled_rounded_rectangle( 50, 25, 350, 575, 10, 10, al_map_rgb(187, 173, 160));
    al_draw_filled_rounded_rectangle(400, 25, 950, 575, 10, 10, al_map_rgb(187, 173, 160));
    ...
    for (i = 0; i < 4; i++) {
        for (j = 0; j < 4; j++) {
            ...
            int g = (tab[i][j] * (r/100) % 255);
            int b = (tab[i][j] * (r+g / 100)) % 255;
            ALLEGRO_COLOR cor = al_map_rgb(r,g,b);
            if (tab[i][j] != 0) {
                
                al_draw_filled_rounded_rectangle(x1, y1, x2, y2, round, round, cor);
                if (tab[i][j] >= 2048)
                    al_draw_textf(fonte, al_map_rgb(212, 175, 55), x1 + 62.5, y1 + 32, ALLEGRO_ALIGN_CENTRE, "%d", tab[i][j]);
                else
